...
def _build_blacklist_two_slice(train_ready):
    ...
    # Reverse pipeline edges: tput_i_t -> tput_j_t1 where j < i (upstream)
    throughputs = ["throughput_1", "throughput_2", "throughput_3"]
    tput_present = [t for t in throughputs if t in nodes]

    for i, t_parent in enumerate(tput_present):
        for t_child in tput_present[:i]:   # all upstream throughputs
...
def run_one(raw_df, fs_method, disc_method, score_name, k, n_bins,
    ...
    """
    Run one configuration. If train_raw/test_raw are provided (k-fold mode),
    use them directly. Otherwise fall back to single 80/20 split.
    """
    ...
    # The additional two-slice blacklist is passed if supported
...
def main():
    ...
    for tgt in TARGETS:
        ...
        # Generate k-fold temporal splits
                ...
                for disc in DISCRETIZATION_METHODS:
                    for sc in SCORES:
                        for n_bins in N_BINS_VALUES:
                            ...
                            # ── Run all folds ────────────────────────────
                            fold_results = []   # list of result dicts per fold
                            ...
                            for fold_idx, (train_raw_fold, test_raw_fold) in enumerate(folds):
                                try:
                                    ...
                                    # Track best fold model — keep alive, only delete non-best
                                    ...
                                    fold_results.append({
                                        ...
                                        "sbn_acc": np.nan, "sbn_f1": np.nan,
                                        "sbn_prec": np.nan, "sbn_rec": np.nan, "sbn_ll": np.nan,
                            ...
                            # Use cols/mb from last successful fold
                            ...
                            # Best tput results from best fold
                            best_fold_r = max(
                                [r for r in fold_results if r.get("error") is None],
                                key=lambda r: r["accuracy"],
                                default={}
                            )
                            all_tput_results = best_fold_r.get("all_tput_results", {})
                            ...
                            row = {
                                ...
                                "n_folds_ok":                n_ok,
                                # Learned DBN — mean ± std across folds
                                ...
                                "static_bn_accuracy":        _mean("sbn_acc"),
                                "static_bn_accuracy_std":    _std("sbn_acc"),
                                "static_bn_f1":              _mean("sbn_f1"),
                                "static_bn_precision":       _mean("sbn_prec"),
                                "static_bn_recall":          _mean("sbn_rec"),
                                "static_bn_log_loss":        _mean("sbn_ll"),
                                "static_bn_log_loss_std":    _std("sbn_ll"),
                                ...
                                "error":                     error_str,
                                # Per-throughput breakdown (from best fold)
                            ...
                            # Offer best-fold model to tracker
                            if best_fold_model is not None and not np.isnan(_mean("accuracy")):
                                row = tracker.offer(
                                    _mean("accuracy"), row, best_fold_model,
                                    k, fs, disc, sc, n_bins
                                )
