...
3. Run [docker-compose_chain.yml](docker-compose_chain.yml) script -> docker compose -f docker-compose_chain.yml up -d
4. To stop the containers: docker stop $(docker ps -q)

DOWN: docker compose -f docker-compose_chain.yml down
BUILD: docker compose -f docker-compose_chain.yml up -d --build

## CHANGE CORES:
curl -X PUT "http://localhost:8080/resource_scaling?cores=2" from 1 to 8
## CHANGE QUALITY: 
curl -X PUT "http://localhost:8080/quality_scaling?data_quality=552" from 100 to 1000

## Prometheus to csv: 
START=$(date -u -v-10M +%FT%TZ)
END=$(date -u +%FT%TZ)
./export_prometheus.sh "$START" "$END" 15s prom_export_last10m

## BN from prometheus
1. export_prometheus.sh -> a folder with metrics 
2. merge_prometheus_csvs -> takes the folder and merges the file into one single csv: baseline_merged.csv
3. bn_learn.py creates the bn_edges.csv and visualize_bn_edges.py creates the bn
