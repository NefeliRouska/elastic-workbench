...
# ============================================================
EVIDENCE_MODE   = "full_with_target"   # was "full_no_target"
...
class Discretizer:
    ...
    def fit(self, df, target_col=None):
        ...
        if self.method == "gmm":
            ...
            for c in self.columns_:
                ...
                n_comp = min(self.n_bins, np.unique(x).shape[0])
                try:
                    gm = GaussianMixture(
    ...
    def transform(self, df):
        ...
        if self.method == "decision_tree":
            for c in self.columns_:
                if c in self.tree_cuts:
                    cuts = self.tree_cuts[c]
                    if len(cuts) == 0:
                        out[c] = 0
                    else:
                        out[c] = np.digitize(
        ...
        if self.method == "gmm":
            for c in self.columns_:
                if c in self.gmm_cuts:
                    cuts = self.gmm_cuts[c]
                    if len(cuts) == 0:
                        out[c] = 0
                    else:
                        out[c] = np.digitize(
...
def select_markov_blanket(train_df, k, score_name, n_bins, disc_method):
    ...
    quick_disc.fit(filtered, target_col=TARGET)
...
def _bulk_compression_frac(train_col_raw, train_col_scaled):
    ...
    bulk_scaled = train_col_scaled[bulk_mask]
    return float(np.mean(np.abs(bulk_scaled) < 0.5))
...
def _normalize(train_fs, test_fs):
    ...
    for c in feature_cols:
        train_col = train_fs[[c]].to_numpy(dtype=float)
        test_col  = test_fs[[c]].to_numpy(dtype=float)
        ...
        best_name, best_frac, best_scaler = None, float("inf"), None
        for name, scaler in candidates.items():
            scaled = scaler.fit_transform(train_col).flatten()
            frac = _bulk_compression_frac(train_col.flatten(), scaled)
            if frac < best_frac:
                best_name, best_frac, best_scaler = name, frac, scaler

        train_fs[c] = best_scaler.transform(train_col).flatten()
        test_fs[c]  = best_scaler.transform(test_col).flatten()
                    ...
                    for sc in SCORES:
                        for n_bins in N_BINS_VALUES:
                            ...
                            for fold_idx, (train_raw_fold, test_raw_fold) in enumerate(folds):
                                try:
                                    ...
                                    fold_results.append({
                                        ...
                                        "legacy_recall":      res_legacy_evidence["recall"],
                                        "legacy_log_loss":    res_legacy_evidence["log_loss"],
                                    ...
                                    is_best_fold = res["f1"] > best_fold_f1
                                    if is_best_fold:
                                        best_fold_f1 = res["f1"]
                                    ...
                                    print(
                                        ...
                                        f"(legacy-ev acc={res_legacy_evidence['accuracy']:.3f})",
                                        flush=True,
                                    )
                            ...
                            row = {
                                ...
                                "dbn_legacy_evidence_f1_std":        _std("legacy_f1"),
                                "dbn_legacy_evidence_precision":     _mean("legacy_precision"),
                                "dbn_legacy_evidence_recall":        _mean("legacy_recall"),
                                "dbn_legacy_evidence_log_loss":      _mean("legacy_log_loss"),
                                "dbn_legacy_evidence_log_loss_std":  _std("legacy_log_loss"),
                                ...
                                "best_fold_f1":              best_fold_f1 if best_fold_f1 > -1.0 else np.nan,
                            ...
                            print(
                                ...
                                f"| legacy_acc={_mean('legacy_accuracy'):.3f} "
        ...
        with pd.option_context("display.max_colwidth", 120):
            print(
                out_sorted.head(50)[[
                    ...
                    "dbn_legacy_evidence_accuracy", "dbn_legacy_evidence_f1",
                    "dbn_legacy_evidence_log_loss",
