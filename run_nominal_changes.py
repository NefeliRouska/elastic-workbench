...
CORES = [1, 3, 7, 8]
QUALITIES = [100, 200, 300, 900, 1000]
#QUALITY_MIN, QUALITY_MAX, QUALITY_STEP = 250, 750, 100
#QUALITIES = list(range(QUALITY_MIN, QUALITY_MAX + 1, QUALITY_STEP))

STABILIZE_SEC = 10     # wait after applying config
MEASURE_SEC   = 60     # measurement window

INPUT_CSV  = Path("share/metrics/metrics.csv")
OUTPUT_CSV = Path("share/metrics/metrics_nominal_changes_perturbated.csv")
# =====================
...
def main():
    ...
    print(f"Total nominal-change configurations: {len(combos)}")

    if OUTPUT_CSV.exists():
        OUTPUT_CSV.unlink()  # start clean

    first_window = True
    for cores, quality in combos:
        tag = f"cores-{cores}_dq-{quality}"
        print(f"\n=== Running nominal change: {tag} ===")
